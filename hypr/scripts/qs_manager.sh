...
SRC_DIR="${WALLPAPER_DIR:-${srcdir:-$HOME/Pictures/Wallpaper}}"
...
if [[ "$ACTION" == "open" || "$ACTION" == "toggle" ]]; then
    ...
    # Stewart desktop buddy — standalone quickshell process, toggled independently
    if [[ "$TARGET" == "stewart" ]]; then
        STEWART_QML="$HOME/.config/hypr/scripts/quickshell/stewart/stewart.qml"
        STEWART_PID=$(pgrep -f "quickshell.*stewart.qml" | head -1)
        if [ -n "$STEWART_PID" ]; then
            kill "$STEWART_PID" 2>/dev/null
        else
            quickshell -p "$STEWART_QML" &
            disown
        fi
        exit 0
    fi
