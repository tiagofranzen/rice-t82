    ...
    delegate: Component {
        PanelWindow {
            ...
            Process {
                id: chassisDetector
                running: true
                command: ["bash", "-c", "if ls /sys/class/power_supply/BAT* 2>/dev/null | grep -q . || [ -d /sys/class/power_supply/qcom-battmgr-bat ]; then echo 'laptop'; else echo 'desktop'; fi"]
                stdout: StdioCollector {
                    onStreamFinished: {
            ...
            property bool isVpnOn: barWindow.vpnStatus === "connected"
            ...
            Timer { id: mprisRestartDelay; interval: 2000; repeat: false; onTriggered: { mprisWatcher.running = true; } }
            Process {
                id: mprisWatcher
                running: true
                command: ["bash", "-c", "dbus-monitor --session \"type='signal',interface='org.freedesktop.DBus.Properties',member='PropertiesChanged',arg0='org.mpris.MediaPlayer2.Player'\" \"type='signal',interface='org.mpris.MediaPlayer2.Player',member='Seeked'\" 2>/dev/null | grep -m 1 'member=' > /dev/null || sleep 5"]
                onExited: {
                    musicForceRefresh.running = false;
                    musicForceRefresh.running = true;
                    running = false;
                    mprisRestartDelay.restart();
                }
            }
                ...
                stdout: StdioCollector {
                    onStreamFinished: {
                        ...
                        if (txt !== "") {
                            try {
                                let data = JSON.parse(txt);
                                if (barWindow.vpnStatus !== data.status) barWindow.vpnStatus = data.status;
                                if (barWindow.vpnIcon !== data.icon) barWindow.vpnIcon = data.icon;
                                if (barWindow.vpnName !== data.name) barWindow.vpnName = data.name;
                            } catch(e) {}
                        }
                        vpnTimer.running = true;
                    }
                }
            }
            Timer { id: vpnTimer; interval: 5000; repeat: false; onTriggered: { vpnPoller.running = false; vpnPoller.running = true; } }
                ...
                stdout: StdioCollector {
                    onStreamFinished: {
                        ...
                        if (txt !== "") {
                            try {
                                let data = JSON.parse(txt);
                                if (barWindow.sysCpu !== data.cpu) barWindow.sysCpu = data.cpu;
                                if (barWindow.sysRam !== data.ram) barWindow.sysRam = data.ram;
                                if (barWindow.sysTemp !== data.temp) barWindow.sysTemp = data.temp;
                            } catch(e) {}
                        }
                        sysinfoWaiter.running = false;
                        sysinfoWaiter.running = true;
                    }
                }
            }
            Process { id: sysinfoWaiter; command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/sysinfo_wait.sh"]; onExited: { sysinfoPoller.running = false; sysinfoPoller.running = true; } }
            ...
            Item {
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        property real targetWidth: 0  // tray icons hidden
                        width: targetWidth
                        Behavior on width { NumberAnimation { duration: 400; easing.type: Easing.OutExpo } }
                    ...
                    Rectangle {
                        ...
                        Row {
                            ...
                            Rectangle {
                                ...
                                property color tempColor: barWindow.sysTemp >= 80 ? mocha.red : barWindow.sysTemp >= 65 ? mocha.peach : mocha.green
                                property color cpuColor: barWindow.sysCpu >= 80 ? mocha.red : barWindow.sysCpu >= 50 ? mocha.peach : mocha.teal

                                property real targetWidth: sysinfoRow.implicitWidth + barWindow.s(24)
                                width: targetWidth
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                                ...
                                property bool initAnimTrigger: false
                                Timer { running: rightContent.showLayout && !sysinfoPill.initAnimTrigger; interval: 175; onTriggered: sysinfoPill.initAnimTrigger = true }
                                opacity: initAnimTrigger ? 1 : 0
                                transform: Translate { y: sysinfoPill.initAnimTrigger ? 0 : barWindow.s(15); Behavior on y { NumberAnimation { duration: 500; easing.type: Easing.OutBack } } }
                                Behavior on opacity { NumberAnimation { duration: 400; easing.type: Easing.OutCubic } }
                                ...
                                Row {
                                    ...
                                    Row {
                                        ...
                                        anchors.verticalCenter: parent.verticalCenter
                                        Text { anchors.verticalCenter: parent.verticalCenter; text: "󰍛"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(15); color: sysinfoPill.cpuColor; Behavior on color { ColorAnimation { duration: 300 } } }
                                        Text { anchors.verticalCenter: parent.verticalCenter; text: barWindow.sysCpu + "%"; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(12); font.weight: Font.Black; color: mocha.text }
                                    }
                                    Row {
                                        spacing: barWindow.s(5)
                                        anchors.verticalCenter: parent.verticalCenter
                                        Text { anchors.verticalCenter: parent.verticalCenter; text: "󰘚"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(15); color: mocha.blue }
                                        Text { anchors.verticalCenter: parent.verticalCenter; text: barWindow.sysRam + "G"; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(12); font.weight: Font.Black; color: mocha.text }
                                    }
                                    Row {
                                        spacing: barWindow.s(5)
                                        anchors.verticalCenter: parent.verticalCenter
                                        Text { anchors.verticalCenter: parent.verticalCenter; text: "󰔄"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(15); color: sysinfoPill.tempColor; Behavior on color { ColorAnimation { duration: 300 } } }
                                        Text { anchors.verticalCenter: parent.verticalCenter; text: barWindow.sysTemp + "°"; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(12); font.weight: Font.Black; color: mocha.text }
                                    }
                                }
                            ...
                            Rectangle {
                                ...
                                Rectangle {
                                    anchors.fill: parent
                                    radius: barWindow.s(10)
                                    opacity: barWindow.isVpnOn ? 1.0 : 0.0
                                    Behavior on opacity { NumberAnimation { duration: 300 } }
                                    gradient: Gradient {
                                ...
                                property real targetWidth: vpnLayoutRow.implicitWidth + barWindow.s(24)
                                width: targetWidth
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                                ...
                                Row {
                                    ...
                                    Text {
                                        anchors.verticalCenter: parent.verticalCenter;
                                        text: barWindow.vpnIcon;
                                        font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(16);
                                        color: barWindow.isVpnOn ? mocha.base : mocha.subtext0
                                    }
                                    Text {
                                        id: vpnText
                                        anchors.verticalCenter: parent.verticalCenter
                                        text: barWindow.isVpnOn ? (barWindow.vpnName !== "" ? barWindow.vpnName : "On") : "Off"
                                        visible: text !== ""
                                        font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(13); font.weight: Font.Black;
                                        color: barWindow.isVpnOn ? mocha.base : mocha.text;
                                        width: Math.min(implicitWidth, barWindow.s(100)); elide: Text.ElideRight
                                    }
                                }
                                MouseArea { id: vpnMouse; hoverEnabled: true; anchors.fill: parent; onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/vpn_toggle.sh"]) }
                            }
                            ...
                            Rectangle { id: batPill;
                                property bool isHovered: batMouse.containsMouse
                                color: isHovered ? Qt.rgba(mocha.surface1.r, mocha.surface1.g, mocha.surface1.b, 0.6) : Qt.rgba(mocha.surface0.r, mocha.surface0.g, mocha.surface0.b, 0.4); 
                                radius: barWindow.s(10); height: sysLayout.pillHeight;
                                clip: true

                                Rectangle { id: animRect5;
                                    anchors.fill: parent
                                    radius: barWindow.s(10)
                                    opacity: 1.0 
                                    Behavior on opacity { NumberAnimation { duration: 300 } }
                    ...
                    // ── Sysinfo button (hidden) ──────────────────────────────────
                    Rectangle {
                        id: sysinfoBtn
                        property bool isHovered: sysinfoMouse.containsMouse
                        property bool isActive:  barWindow.activeWidget === "sidepanel"
                        ...
                        MouseArea {
                            ...
                            anchors.fill: parent
                            hoverEnabled: true
                            onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/qs_manager.sh toggle sidepanel"])
                        }
                    }
