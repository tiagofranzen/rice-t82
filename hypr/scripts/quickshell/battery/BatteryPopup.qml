...
Item {
    ...
    Process {
        id: sysPoller
        command: ["bash", "-c", 
            "BAT=/sys/class/power_supply/qcom-battmgr-bat; N=$(cat $BAT/energy_now 2>/dev/null||echo 0); F=$(cat $BAT/energy_full 2>/dev/null||echo 1); [ $F -gt 0 ] && echo $((N*100/F)) || echo 0; " +
            "cat /sys/class/power_supply/qcom-battmgr-bat/status 2>/dev/null | head -n1 || echo 'Unknown'; " +
