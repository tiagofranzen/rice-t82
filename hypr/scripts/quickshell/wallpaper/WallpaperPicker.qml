...
Item {
    ...
    Timer {
        id: autoCloseTimer
        interval: 800
        onTriggered: Quickshell.execDetached(["bash", "-c", "echo 'close' > /tmp/qs_widget_state"])
    }
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        // autoCloseTimer removed: picker stays open until Escape
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    ( matugen image "$FINAL_THUMB" || true; bash "$RELOAD_SCRIPT" "$DEST_FILE" || true ) &
                `;
        ...
        const fullScript = `
            ...
            ${wallpaperCmd}
            ( matugen image "${escThumb}" || true; bash "${escReload}" "${escOriginal}" || true ) &
        `;
    ...
    Shortcut { sequence: "Escape"; enabled: !window.isApplying; onActivated: { if (window.currentFilter === "Search") { window.currentFilter = "All"; } else { Quickshell.execDetached(["bash", "-c", "echo 'close' > /tmp/qs_widget_state"]) } } }
